...
def create_channel_check_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Guruhga a'zo bo'lish", url=f"https://t.me/{CHANNEL_USERNAME[1:]}")],
...
async def start_command(message: types.Message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        await message.answer("Assalomu alaykum, tanlovda ishtirok etish uchun avval kanalga a'zo bo'ling va 'Obuna bo'ldim' tugmasini bosing.",
...
async def provide_referral_link(message: types.Message):
    ...
    await message.answer(f"🤝Tabriklaymiz, siz tanlovda qatnashish uchun muvaffaqiyatli ro‘yxatdan o‘tdingiz."
                         f" \n\n 👩‍💻Assalomu alaykum \n Xurmatli gruppa aʼzolari\n Bu gruppa xalq uchun ochilgan \n Gruppada xaqqoniy oʻyin oʻtkazamiz.\n Kim eng koʻp refferal yig'ib g'olib boʻlganga pul mukofoti beriladi. \n\n 1-o'rin - 300000 so'm \n 2-o'rin - 200000 so'm \n 3-o'rin - 100000 so'm \n\n Sizning refferal linkingiz: {referral_link}",
...
async def show_user_results(message: types.Message):
    ...
    await message.answer(f"Siz {user_referrals} bal")
...
async def show_ranking(message: types.Message):
    ...
    for rank, (user_id, ref_list) in enumerate(ranking, start=1):
        ...
        ranking_message += f"{rank}. {user_name} - {len(ref_list)} ball.\n"
...
async def show_rules(message: types.Message):
    rules = """
    1.Tanlovda ishtirok etish uchun guruhga a'zo bo'ling!
